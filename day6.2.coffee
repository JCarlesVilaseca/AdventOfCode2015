...
process.stdin.on 'end', ->
	...
	for y in [0..HEIGHT-1]
		for x in [0..WIDTH-1]
			coord = y * WIDTH + x
			ons = ons + if grid[coord] then grid[coord] else 0
...
turnOnFn = (y1,x1,y2,x2)->
	for y in [y1..y2]
		for x in [x1..x2]
			coord = y * WIDTH + x
			grid[coord] = if grid[coord] then grid[coord] + 1 else 1

turnOffFn = (y1,x1,y2,x2)->
	for y in [y1..y2]
		for x in [x1..x2]
			coord = y * WIDTH + x
			grid[coord] = if grid[coord] then grid[coord] - 1 else 0
...
toggleFn = (y1,x1,y2,x2)->
	for y in [y1..y2]
		for x in [x1..x2]
			...
			grid[coord] = if grid[coord] then grid[coord] + 2 else 2
